...
class Filter extends \Opencart\System\Engine\Model {
    ...
    public function getManufacturers(array $ctx, bool $show_count = false, array $active = []): array {
        $count_col = $show_count ? ', COUNT(DISTINCT p.product_id) AS `count`' : '';
        $join = $this->contextJoin($ctx);
        $where = $this->contextWhere($ctx);

        $filters = $this->buildFilterCondition($active, ['manufacturer']);
        $join .= $filters['joins'];
        $where .= $filters['where'];
    ...
    public function getSubCategories(array $ctx, bool $show_count = false, array $active = []): array {
        if ($ctx['type'] !== 'category') return [];
        $count_col = $show_count ? ', COUNT(DISTINCT p2c2.product_id) AS `count`' : '';
        $lang_id = (int)$this->config->get('config_language_id');
        
        $filters = $this->buildFilterCondition($active, ['category']);
        $join = $filters['joins'];
        $where = $filters['where'];

        // We need to join product table to apply filters
        $sql = "SELECT c.category_id AS `id`, cd.name {$count_col} FROM `" . DB_PREFIX . "category` c INNER JOIN `" . DB_PREFIX . "category_description` cd ON (c.category_id = cd.category_id AND cd.language_id = '{$lang_id}') LEFT JOIN `" . DB_PREFIX . "product_to_category` p2c2 ON (c.category_id = p2c2.category_id) LEFT JOIN `" . DB_PREFIX . "product` p ON (p2c2.product_id = p.product_id AND p.status = '1' {$where}) {$join} WHERE c.parent_id = '" . (int)$ctx['id'] . "' AND c.status = '1' GROUP BY c.category_id ORDER BY c.sort_order ASC, cd.name ASC";
        
        $query = $this->db->query($sql);
        return $query->rows;
    }

    public function getAttributeValues(array $ctx, int $attr_id, bool $show_count = false, array $active = []): array {
        $join = $this->contextJoin($ctx);
        $where = $this->contextWhere($ctx);
        $lang_id = (int)$this->config->get('config_language_id');
        $count_col = $show_count ? ', COUNT(DISTINCT p.product_id) AS `count`' : '';

        $filters = $this->buildFilterCondition($active, ['attribute_' . $attr_id]);
        $join .= $filters['joins'];
        $where .= $filters['where'];

        $query = $this->db->query("SELECT DISTINCT pa.text AS `name` {$count_col} FROM `" . DB_PREFIX . "product_attribute` pa INNER JOIN `" . DB_PREFIX . "product` p ON (pa.product_id = p.product_id) {$join} WHERE pa.attribute_id = '" . (int)$attr_id . "' AND pa.language_id = '{$lang_id}' AND p.status = '1' AND pa.text != '' {$where} GROUP BY pa.text ORDER BY pa.text ASC");
        $rows = [];
        foreach ($query->rows as $row) { $rows[] = ['id' => $row['name'], 'name' => $row['name'], 'count' => $row['count'] ?? null]; }
        return $rows;
    }

    public function getOptionValues(array $ctx, int $option_id, bool $show_count = false, array $active = []): array {
        $join = $this->contextJoin($ctx);
        $where = $this->contextWhere($ctx);
        $lang_id = (int)$this->config->get('config_language_id');
        $count_col = $show_count ? ', COUNT(DISTINCT p.product_id) AS `count`' : '';

        $filters = $this->buildFilterCondition($active, ['option_' . $option_id]);
        $join .= $filters['joins'];
        $where .= $filters['where'];

        $query = $this->db->query("SELECT DISTINCT ov.option_value_id AS `id`, ovd.name {$count_col} FROM `" . DB_PREFIX . "product_option_value` pov INNER JOIN `" . DB_PREFIX . "product` p ON (pov.product_id = p.product_id) INNER JOIN `" . DB_PREFIX . "option_value` ov ON (pov.option_value_id = ov.option_value_id) INNER JOIN `" . DB_PREFIX . "option_value_description` ovd ON (ov.option_value_id = ovd.option_value_id AND ovd.language_id = '{$lang_id}') {$join} WHERE pov.option_id = '" . (int)$option_id . "' AND p.status = '1' {$where} GROUP BY ov.option_value_id ORDER BY ov.sort_order ASC, ovd.name ASC");
        return $query->rows;
    }

    public function getOcFilterValues(array $ctx, int $filter_group_id, bool $show_count = false, array $active = []): array {
        $join = $this->contextJoin($ctx);
        $where = $this->contextWhere($ctx);
        $lang_id = (int)$this->config->get('config_language_id');
        $count_col = $show_count ? ', COUNT(DISTINCT p.product_id) AS `count`' : '';

        $filters = $this->buildFilterCondition($active, ['filter_group_' . $filter_group_id]);
        $join .= $filters['joins'];
        $where .= $filters['where'];

        $query = $this->db->query("SELECT DISTINCT f.filter_id AS `id`, fd.name {$count_col} FROM `" . DB_PREFIX . "product_filter` pf INNER JOIN `" . DB_PREFIX . "product` p ON (pf.product_id = p.product_id) INNER JOIN `" . DB_PREFIX . "filter` f ON (pf.filter_id = f.filter_id AND f.filter_group_id = '" . (int)$filter_group_id . "') INNER JOIN `" . DB_PREFIX . "filter_description` fd ON (f.filter_id = fd.filter_id AND fd.language_id = '{$lang_id}') {$join} WHERE p.status = '1' {$where} GROUP BY f.filter_id ORDER BY f.sort_order ASC, fd.name ASC");
        return $query->rows;
    }

    public function buildFilterCondition(array $active, array $exclude = []): array {
        $joins = '';
        $where = '';
        $lang_id = (int)$this->config->get('config_language_id');
        
        if (!empty($active['manufacturer_ids']) && !in_array('manufacturer', $exclude)) {
            $ids = implode(',', array_map('intval', $active['manufacturer_ids']));
            $where .= " AND p.manufacturer_id IN ({$ids})";
        }
        
        if (!empty($active['category_ids']) && !in_array('category', $exclude)) {
        ...
        if (!empty($active['attr']) && is_array($active['attr'])) {
            foreach ($active['attr'] as $attr_id => $values) {
                if (in_array('attribute_' . $attr_id, $exclude)) continue;
        ...
        if (!empty($active['opt']) && !in_array('option', $exclude)) {
            // Note: If we had option_id in exclude, we'd need more granular check. 
            // For now, if 'option' is in exclude, we exclude all options.
            // Better: exclude by option_id if available.
        ...
        // Granular option exclude
        if (!empty($active['opt'])) {
            // This would require knowing which option_value belongs to which option_id.
            // For now let's keep it simple or slightly more complex if needed.
        }

        if (!empty($active['filter_ids']) && !in_array('filter', $exclude)) {
        ...
        if ((!empty($active['price_min']) || !empty($active['price_max'])) && !in_array('price', $exclude)) {
            $cg_id = (int)$this->config->get('config_customer_group_id');
            $price_expr = "COALESCE((SELECT (CASE WHEN ps.type = 'P' THEN (p.price - (p.price * (ps.price / 100))) WHEN ps.type = 'S' THEN (p.price - ps.price) ELSE ps.price END) FROM `" . DB_PREFIX . "product_discount` ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '{$cg_id}' AND ps.quantity = '1' AND ps.special = '1' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1), COALESCE((SELECT (CASE WHEN pd.type = 'P' THEN (p.price - (p.price * (pd.price / 100))) WHEN pd.type = 'S' THEN (p.price - pd.price) ELSE pd.price END) FROM `" . DB_PREFIX . "product_discount` pd WHERE pd.product_id = p.product_id AND pd.customer_group_id = '{$cg_id}' AND pd.quantity = '1' AND pd.special = '0' AND ((pd.date_start = '0000-00-00' OR pd.date_start < NOW()) AND (pd.date_end = '0000-00-00' OR pd.date_end > NOW())) ORDER BY pd.priority ASC, pd.price ASC LIMIT 1), p.price))";
        ...
        if (!empty($active['stock']) && !in_array('stock', $exclude)) {
